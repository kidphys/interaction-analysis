...
def get_account_id():
    raw = st.query_params.get('accountId')
    if raw is None:
        return DEFAULT_ACCOUNT_ID
    try:
        return int(raw)
    except (TypeError, ValueError):
        st.error(f"Invalid accountId in URL: {raw!r}. Falling back to default.")
        return DEFAULT_ACCOUNT_ID
...
def get_data(time_filter, account_id):
    prefix = f'admin_{account_id}_'
    # Fetch sessions and presentations by account (subquery, no huge IN clause)
    sessions, presentations, user_count_df = parallelize([
        (fetch_sessions_by_account, (account_id,), {'prefix': prefix}),
        (fetch_presentations_by_account, (account_id,), {'prefix': prefix}),
        (fetch_user_count_by_account, (account_id,), {'prefix': prefix}),
    ])
    ...
    users, participants, answers, presentation_names = parallelize([
        (fetch_users, (active_user_ids,), {'prefix': prefix}),
        (fetch_participants_by_presentations, (all_session_ids,), {'prefix': prefix}),
        (fetch_answers_by_presentations, (all_session_ids,), {'prefix': prefix}),
        (fetch_presentations_by_ids, (all_session_ids,), {'prefix': prefix}),
    ])
...
def render_dashboard():
    ...
    st.title(f'Admin Analytics — Account {account_id}')
    st.markdown("""
    <style>
    ...
    with st.spinner('Loading data...'):
        data = get_data(time_filter, account_id)
