...
cols = ['ID', 'Presentationid', 'Interaction Source', 'Numberofreactions', 'Reactiontype',
       'Slideid', 'Audienceid', 'Createdat', 'Updatedat', 'Slideoptions', 'Slidetitle', 'Slide Description', 'Title', 'Slideorder', 'Slidetypenormalized', 'Poll Vote']
...
filtered_df = df[df['Audienceid'].isin(audience_ids)].copy()

full = df.groupby(['Slideid', 'Slideorder', 'Slidetitle', 'Slidetypenormalized'])['Createdat'].size().reset_index().sort_values(by='Slideorder')
data = filtered_df.groupby(['Slideid', 'Slideorder', 'Slidetitle', 'Slidetypenormalized'])['Createdat'].size().reset_index().sort_values(by='Slideorder')
...
selected_slide_type = st.selectbox('Select Slide Type', slide_type_options)
